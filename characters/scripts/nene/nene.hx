...
function onUpdate(elapsed:Float) {
	...
	if (viz != null)
		viz.setVisible(character.visible);
...
function onNoteHit(note:Dynamic) {
	if (note != null)
		moveByNoteKind(note.noteType);
}

function onNoteMiss(note:Dynamic) {
	if (note != null)
		moveByNoteKind(note.noteType);
}
...
function onEvent(name:String, v1:String, v2:String, time:Float):Bool {
	...
	if (lname == 'camera follow' || lname == 'camera' || lname == 'camera focus' || lname == 'focus camera' || lname == 'focus') {
...
function movePupilsLeft():Void {
	if (pupil != null && pupil.animation != null)
		pupil.animation.play('', true, false, 0);
}

function movePupilsRight():Void {
	if (pupil != null && pupil.animation != null)
		pupil.animation.play('', true, false, 17);
}

function moveByNoteKind(kind:String):Void {
	if (kind == 'weekend-1-lightcan')
		movePupilsLeft();
	else if (kind == 'weekend-1-cockgun')
		movePupilsRight();
}
...
function setTrainPassing(value:Bool):Void {
	trainPassing = value;
}
...
function transitionState():Void {
	...
	var hp:Float = (health != null) ? health : 1.0;
	...
	switch (currentState) {
		...
		case 5: // HAIR_BLOWING
			if (!trainPassing) {
				...
				character.playAnim('hairFallNormal', true);
				animationFinished = false;
			} else if (animationFinished) {
				character.playAnim('hairBlowNormal', true);
				animationFinished = false;
			}
		...
		case 7: // HAIR_BLOWING_RAISE
			if (!trainPassing) {
				...
				character.playAnim('hairFallKnife', true);
				animationFinished = false;
			} else if (animationFinished) {
				character.playAnim('hairBlowKnife', true);
				animationFinished = false;
			}
